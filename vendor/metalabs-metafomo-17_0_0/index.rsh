...
export const main = Reach.App(() => {
  ...
  const getNewPrice = (price, unitPrice) => price + unitPrice;

  const getOldPrice = (price, unitPrice) => price - unitPrice;
  ...
  const getDiscountPrice = (who) => {
    ...
    if (level > maxDiscountLevel) {
      return 1000000000; // should never happen. Only for verifier. TODO check if we can do better
    } else {
  ...
  const getParticipantStats = (p) =>
    ParticipantStats.fromObject({
      addr: p,
  ...
  const [
    ...
    .invariant(
      balance() == totalRaised - paidToFunder &&
        balance(nftPrize) == 1 &&
        fomoToken.destroyed() == false &&
        fomoToken.supply() == UInt.max
    )
    .while(keepGoing)
    .paySpec([fomoToken])
    .api(
    ...
    .api(Api.getParticipantStats, (callback) => {
      const participantStats = getParticipantStats(this);
      callback(participantStats);
  ...
  Buyer.only(() => {
    interact.showOutcome(winner, ticketsSold, totalRaised, price, paidToFunder);
  });
  commit();
