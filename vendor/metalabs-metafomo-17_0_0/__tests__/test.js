const { stdlib, parseBigNumber, checkEvents, convertBns } = require("@metalabsog/common");
const { init, deploy } = require("../index.mjs");

let playerAccs, playerCtcs, metafomoToken, playerCtcConnection, creatorCtc;
...
async function getView() {
  ...
  const [_, object] = await ctc.views.Fomo.info();
...
async function getLocalView(addr) {
  const ctc = playerCtcs[0];
  // TODO should it be () ()?
  const [_, object] = await ctc.views.Fomo.participantInfo(addr);
  return convertBns(object);
}
...
beforeEach(async () => {
  ...
  const [_, initialInfo] = await playerCtcs[0].views.Fomo.info();
  ...
  playerCtcConnection = playerCtcs[0].p.Buyer({});
});

afterEach(async () => {
  console.log(playerCtcConnection);
...
test("events are emitted", async () => {
  ...
  const buyDiscountEvents = [[[1]], [[1]]];

  await checkEvents(e.updateDiscountLevel, buyDiscountEvents, playerAccs);

  const buyTimeReductionEvents = [[[1]], [[]]];

  checkEvents(e.updateTimeReductionLevel, buyTimeReductionEvents, playerAccs);
});
