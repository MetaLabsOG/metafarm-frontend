    ...
    try {
        ...
        try {
            ...
            const transformedData: Priced<LPTokenInfo> = {
                ...
                asset1Reserve: BigInt(Math.round(backendData.asset1_reserve_micros || 0)),
                asset2Reserve: BigInt(Math.round(backendData.asset2_reserve_micros || 0)),
                totalLiquidity: BigInt(Math.round(backendData.issued_tokens_micros || 0)),
...
function backendLpToInfo(lpState: BackendLPTokenInfo, asset: Asset | null): Priced<LPTokenInfo> {
    ...
    return {
        ...
        asset1Reserve: BigInt(Math.round(lpState.asset1_reserve_micros || 0)),
        asset2Reserve: BigInt(Math.round(lpState.asset2_reserve_micros || 0)),
        totalLiquidity: BigInt(Math.round(lpState.issued_tokens_micros || 0)),
