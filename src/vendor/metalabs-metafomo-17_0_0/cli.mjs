import { printObjectWithBigNumbers, call, parseBigNumber, stdlib } from "@metalabsog/common";
import inquirer from "inquirer";
import { deploy, init } from "./index.mjs";
...
const { contractId, playerCtcs } = await deploy(creatorAcc, playerAccs, nftPrize);
...
const playerInteract = (player) => ({
  ...
  showPurchase: (addr, lastPrice, newPrice) => {
    console.log(
      `Player ${player} showPurchase`,
      addr,
      parseBigNumber(lastPrice),
      parseBigNumber(newPrice)
    );
  },
