...
export async function collectFees() {
    ...
    stdlib.setProviderByName(IS_MAINNET ? 'MainNet' : 'TestNet');

    const cometaUrl = IS_MAINNET ? 'https://api.cometa.farm' : 'https://api.testnet.cometa.farm';

    for (const type of Object.keys(CONTRACT_BACKENDS)) {
        const contractList = await axios.get(`${cometaUrl}/contracts?type=${type}`).then(({ data }) => data);
