...
| MF-117 | Fix Pera connect P0: complete iOS relay wake-listener (corrects MF-112) | done | critical | safety | `installRelayerWakeListeners` `wake()` dropped the `relayer.connected` guard — that getter reads `socket.readyState===1`, which stays OPEN ~5s during iOS WK WebView suspension, so `restartTransport` was skipped exactly when Pera's `session_settle` arrives → "dApp is not responding". MF-112 shipped the listener (990fdff) but the guard neutralized it. **NOTE: this was NOT the Pera root cause** — device testing showed Pera also fails on desktop QR (no iOS suspension), so the real fix is MF-118. This change remains a valid iOS-robustness fix for Defly and is kept. Also added `optionalNamespaces` to both `connect()` calls and replaced the broken WC v1 `SessionTypes` import with an inline `WCSession` type. `walletConnectService.ts`. Commit `5c05af1`. 2026-05-29 |
| MF-118 | Fix Pera connect (real root cause): route Pera through native @perawallet/connect | done | critical | safety | Device testing proved Pera fails EVERYWHERE (iOS/Android/desktop-QR) while Defly works everywhere through the identical raw WC v2 code. Root cause: Pera's app connects dApps over **WC v1 on Pera's own bridge** (`@perawallet/connect`); it no longer reliably handles a generic WC v2 proposal (freezes → "App is not responding"). Every working Algorand dApp uses `@perawallet/connect` for Pera. Fix: add `@perawallet/connect@1.3.5` (last release with algosdk `^2.1.0` peer — compatible with our v2; 1.4.0+ needs v3/Reach blocker), new `peraWalletService.ts` adapter (decode base64→`algosdk.Transaction`, `signTransaction([group], signerAddr)`, merge compacted signed array back like @txnlab/use-wallet, encode→base64), route `WalletType='WalletConnect'`→Pera native in `customWalletFallback.ts`. **Defly untouched** (stays raw WC v2). algosdk runtime: Pera 1.3.5 calls only `encodeUnsignedTransaction`, present in 2.11.0. **Connect VERIFIED** by owner on iOS + desktop QR + browser (the 4-week "App is not responding" desktop-QR symptom is gone). Live bundle `main.6b6b3ea1.js` confirmed to contain Pera code. Signing (mixed lsig groups) high-confidence (mirrors use-wallet `shift()` merge) — confirm with one real stake/unstake. Commit `9f990f2`. 2026-05-29 |
